...
import { executeAction, removeCellPossibility, setCellValue } from "../helpers/sudoku.helpers";
...
export function tryToSolve(sudokuState: SudokuStoreState, maxAlgorithmLevel?: number, returnActionsToCaller?: boolean): SudokuCellState[][]{
  ...
  while(tryAgain){
    ...
    if(suggestedActions.length>0){
      if(returnActionsToCaller){
        postMessage(suggestedActions);
      }
